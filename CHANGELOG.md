...
- **`vtx-select` supports true remote search** (`data-searchable data-ajax-url`) - queries the server per keystroke (debounced 220ms) instead of preloading the full option list once and filtering client-side. Use it for any select whose option list can reach into the thousands (a brand/product/user picker over a large table); render only the currently-selected option inline (if any) so its label shows without waiting on a fetch. See `docs/javascript-components.md`'s vtx-select section for the endpoint contract.
- **`ModuleLoader::frontAssets()` now accepts an ordered array of module slugs, not just a single one** - lets one page load combined front CSS/JS from several modules that share its markup (e.g. a tabbed member workspace mixing several modules' own widgets), instead of being limited to the page's single auto-derived "owning" module. `ThemeEngine::render()` reads an optional `front_asset_modules` data key to drive this; the existing automatic single-module derivation from the view path (`modules/{slug}/front/...`) is unchanged and still applies whenever that key isn't set. Ported from Carikno, where it was needed for a workspace page combining Members with two other modules. `App/CMS/ModuleLoader.php`, `App/Theme/ThemeEngine.php`.
- **15 new icon glyphs**: `pi-bolt`, `pi-eye-slash`, `pi-map-marker`, `pi-play`, `pi-refresh`, `pi-sparkles`, `pi-spinner`, `pi-unlink`, plus `pi-envelope`, `pi-equals`, `pi-exclamation-triangle`, `pi-info-circle`, `pi-times`, `pi-warning`, `pi-warning-circle`. `Public/assets/css/icons.css` bumped to `?v=3`. Ported from Phuse 1.3.3.
- **New test: `tests/App/CMS/IconAssetCoverageTest.php`** - fails if any `pi-*` class referenced in `App`/`Core`/`Public` has no matching glyph defined in `icons.css`. Ported from Carikno. It immediately caught a real pre-existing gap here: `pi-envelope` (mail settings/contact-reply views), `pi-equals` (nav builder drag handle), `pi-exclamation-triangle`/`pi-warning` (SEO notice, 2FA backup codes), `pi-info-circle` (nav builder hint), `pi-times` (webhook log status), and `pi-warning-circle` (mail settings notice) were all referenced with no glyph ever defined for them, so they rendered invisible. Fixed by adding the missing glyphs above rather than leaving the test red.

### Changed

- **Updated the framework to Phuse 1.3.3** - pulls in the admin/AJAX query-cache staleness fix and 15 new icon glyphs called out below.

### Fixed

- **Admin and AJAX screens could serve stale query-cache results** - `Core\Model::initializeQueryCache()` built its cache config from hardcoded `CACHE_LIFETIME`/`CACHE_DIRECTORY` constants plus an inline array, with no way for a request to opt out - every `SELECT`/`SHOW`/`DESCRIBE`/`EXPLAIN` query was cached for a flat 1 hour regardless of context. Confirmed on a consuming application (Carikno): admin list screens kept showing content up to an hour stale after a write from a background worker or another pod. `Config\Database::getCacheConfig()` now accepts the current request URI (defaulting to `$_SERVER['REQUEST_URI']`) and disables caching whenever the path contains a complete `/admin/` segment. `Core\Model` now pulls its cache config from `Config\Database::getCacheConfig()` instead of the removed hardcoded constants. Ported from Phuse 1.3.3.
- **A `vtx-select` field with a blank-value placeholder option (e.g. "- None -") had no way to be cleared back to blank once a real value was selected** - `readNativeOptions()` unconditionally dropped any `<option value="">` from the list it renders, on the assumption a blank option only ever exists to show placeholder text, never to be actively re-selected. Confirmed on Carikno: Categories' "Parent Category" field couldn't be reset to top-level after already having a parent. The blank option is now kept in the rendered list like any other entry (clickable, or disabled if the option itself is disabled) - affects every optional-relation dropdown built this way, not just one form.
- **The admin HTML minifier could crash rendering any sufficiently large page** - `TypeError: preg_replace(): Argument #3 ($subject) must be of type array|string, null given`. `Core\Utilities\Text\HTML::minifyWhitespace()` (and several sibling methods) chain multiple `preg_replace()`/`preg_replace_callback()` calls over the full rendered page; on large-enough input one of those calls can hit PHP's internal PCRE backtrack limit and return `NULL` rather than throwing, and every following call in the chain then crashed trying to use that `NULL` as its subject. Confirmed on Carikno: a form with two dropdowns totaling ~13,000 `<option>` tags. Every such call now falls back to its unmodified input on `NULL`, so an oversized page degrades (slightly less compact whitespace) instead of crashing outright.
- **A `[data-ajax-panel]` form declaring `method="POST"` was silently resubmitted as GET** - `admin.js`'s generic ajax-nav handler (built for GET filter/search forms) intercepted the submit of ANY form carrying `data-ajax-panel`, regardless of its actual `method`, and rewrote the request into a GET query string - a POST-only action route (e.g. a "Sync"/"Run now" button) would 404. The handler now yields to forms also marked `data-ajax-form`, which already POST correctly via `VtxAjax.postForm()`. Confirmed live on Carikno (Shipping Estimator's "Sync Stale" button).
- **`Router::match()` logged "Accessing route: ..." and "Route matching took: ... seconds" on every single successfully-matched request** - unconditional, regardless of environment or outcome, drowning out real errors in the shared log file on any site with meaningful traffic. Now only logs on the failure path ("No matching route found..."), which is the case actually worth knowing about. `Core/Router.php`.
- **`Core\Base`'s own copy of the config object could never read a plain top-level config value** (e.g. `$this->config->timezone`) - `Container::get('config')` resolves `Config::class` via reflection, which just instantiates the service directly rather than calling its `get()` method, so `Base`'s `$this->config` ends up being the raw service object (only `getEnv()`/`get()`/`set()`), not the processed data object every other class gets via `(new Config)->get()` (`Core\Controller`). Property reads on it silently returned `null` instead of erroring, so the gap stayed invisible until Carikno's `Asia/Jakarta` timezone setting depended on it and was found live to always resolve to UTC instead. `Core\Config` now proxies direct property reads to its underlying config array, so a raw service instance works either way. `Core/Config.php`.
